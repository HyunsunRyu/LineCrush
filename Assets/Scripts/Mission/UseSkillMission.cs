...
public class UseSkillMission : MissionBase
{
    ...
    public override bool IsCleared(int level)
    {
        ...
        if (table != null)
        {
            return (MissionManager.GetInstance().useSkillCount >= table.value);
        }
        return false;
    }
    ...
    public override float GetMissionState(int level)
    {
        ...
        if (table != null)
        {
            float rate = (float)MissionManager.GetInstance().useSkillCount / (float)table.value;
